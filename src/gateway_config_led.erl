...
-define(BLUEZ_MEMBER_PROPERTIES_CHANGED,
	"PropertiesChanged").

%% Add by Yang, hiot led control
-define(LED_SERVICE, "org.hiot.led").

-define(LED_OBJECT_PATH, "/org/hiot/led").

-define(LED_SET_COLOR, "SetColor").

-define(LED_DST, ?LED_SERVICE ++ "." ++ ?LED_SET_COLOR).
...
%% API
-export([lights_event/1, lights_info/0]).
...
-record(state,
	{handle = undefined  :: lp5562:state() | undefined,
	 state  :: led_state(), off_file  :: string(),
	 pairable_signal  :: ebus:filter_id(),
	 miner_proxy  :: ebus:proxy(),
	 diagnostics = []  :: [{string(), string()}],
	 diagnostics_timeout = make_ref()  :: reference(),
	 led_proxy  :: ebus:proxy()}).

lights_info() -> gen_server:call(?MODULE, lights_info).
...
init([Bus]) ->
    ...
    OffFile = application:get_env(led, lights_off,
				  "/tmp/gateway_lights_off"),
    InitLedState = case filelib:is_regular(OffFile) of
		     true -> disable;
		     _ -> undefined
		   end,
    LedPath = application:get_env(led, path,
				  "/sys/bus/i2c/devices/1-0030"),
    LedHandle = case file:read_file_info(LedPath) of
		  {ok, _} ->
		      {ok, LS} = lp5562:init(LedPath), self() ! init_led, LS;
		  _ ->
    ...
    {ok, BluezProxy} = ebus_proxy:start_link(Bus,
					     ?BLUEZ_SERVICE, []),
    {ok, PairableSignal} =
	ebus_proxy:add_signal_handler(BluezProxy,
				      ?BLUEZ_OBJECT_PATH,
				      ?DBUS_PROPERTIES((?BLUEZ_MEMBER_PROPERTIES_CHANGED)),
				      self(), pairable_signal),
    {ok, MinerProxy} = ebus_proxy:start_link(Bus,
					     ?MINER_APPLICATION_NAME, []),
    %%  Add by Yang
    {ok, LedProxy} = ebus_proxy:start_link(Bus,
					   ?LED_SERVICE, []),
...
handle_info(init_led, State = #state{}) ->
    ...
    DiagnosticsTimer =
	erlang:send_after(?DIAGNOSTICS_TIMEOUT, self(),
			  diagnostics_timeout),
    NewState = State#state{diagnostics_timeout =
			       DiagnosticsTimer},
    {noreply,
     handle_led_event(p2p_led_event(NewState#state.diagnostics),
		      NewState)};
handle_info({ebus_signal, _Path, Signal, Msg},
	    State = #state{pairable_signal = Signal}) ->
    case ebus_message:args(Msg) of
      {ok,
       [?GATT_ADVERTISING_MANAGER_IFACE,
	#{"ActiveInstances" := Value}, _]} ->
	  case Value of
	    0 -> {noreply, handle_led_event(stop_advert, State)};
	    _ -> {noreply, handle_led_event(start_advert, State)}
	  end;
      _ -> {noreply, State}
    end;
...
handle_info(diagnostics_timeout, State = #state{}) ->
    Diagnostics =
	gateway_config:diagnostics(State#state.miner_proxy),
    State1 = handle_led_event(p2p_led_event(Diagnostics),
			      State#state{diagnostics = Diagnostics}),
    DiagnosticsTimer =
	erlang:send_after(?DIAGNOSTICS_TIMEOUT, self(),
			  diagnostics_timeout),
    {noreply,
     State1#state{diagnostics_timeout = DiagnosticsTimer}};
...
-spec p2p_led_event([{string(), string()}]) -> online |
					       offline.

p2p_led_event(Diagnostics) ->
    case proplists:get_value("connected", Diagnostics, "no")
	   == "yes"
	of
      true -> online;
      _ -> offline
...
update_off_file(State = #state{state = disable}) ->
    file:write_file(State#state.off_file, <<>>), State;
update_off_file(State = #state{}) ->
    file:delete(State#state.off_file), State.

-spec update_led_state(led_event(),
		       #state{}) -> #state{}.

update_led_state(Event,
...
update_led_state(enable, State = #state{}) ->
    update_off_file(update_led_state(p2p_led_event(State#state.diagnostics),
				     State#state{state = undefined}));
update_led_state(_, State = #state{state = disable}) ->
    %% Ignore all events during disabled state
    State;
%% Start/Stop Advertising
update_led_state(start_advert,
		 State = #state{state = {advert, _}}) ->
...
-spec handle_led_event(Event :: term(),
		       #state{}) -> #state{}.

handle_led_event(Event, State = #state{}) ->
    case update_led_state(Event, State) of
      NewState
	  when NewState#state.state == State#state.state ->
	  NewState;
...
led_set_color( _Color, State = #state{ handle = undefined } ) -> 
    lager : info ( "Would have set LED: ~p" , [ _Color ] ) , State ; 

led_set_color(_Color , State ) -> 
    %% lp5562:set_color( Color , State#state.handle ),
    %% Add by Yang call led_proxy,
    Value = case ebus_proxy:call( 
        State#state.led_proxy , 
        ?LED_OBJECT_PATH , 
        ?LED_DST , 
        [byte , byte , byte] , _Color ) of 
        {ok, feedback} -> 
            lager:info("set_color feedback:~p", [feedback]),
            0;
        {ok, []} ->
            lager:info("set_color empty feedback []"),
            1;
        {error, Error} ->
            lager:warning("Failed to call led set_color:~p", [Error]),
            -1
    end, 
